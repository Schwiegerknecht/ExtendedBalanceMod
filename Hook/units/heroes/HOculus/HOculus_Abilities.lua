...
Ability.HOculusSacrifice01.AreaHeal = function(self, unit, data)
    if data.Amount > self.MinHit then
        ...
        -- local entities = GetEntitiesInSphere(data.Group, pos, self.HealRadius )
        -- entities = EntityCategoryFilterDown( categories.ALLUNITS - categories.UNTARGETABLE, entities )
...
Ability.HOculusSacrifice02.AreaHeal = function(self, unit, data)
    if data.Amount > self.MinHit then
...
Ability.HOculusSacrifice03.AreaHeal = function(self, unit, data)
    if data.Amount > self.MinHit then
        Buff.ApplyBuff(unit, 'HOculusSacrificeDebuff01', unit)
